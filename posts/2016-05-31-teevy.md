...
We can also using `json :: ToJSON a => a -> ActionM ()` instead of `html`. As you can see we need to have a `ToJSON` (encoder) type class instance in scope which is the opposite of `FromJSON` (decoder). I won't go into detail, but you can make a instance by using `instance ToJSON User`. For example you could write `json fetchedUsers` where `fetchedUsers :: [User]` to return a array of users.
