...
Two years ago I joined a few friends which were working on a little hobby project. Creating a website which allowed you to track your watch progression of your favorite television series. The project was called [Teevy](https://teevy.co) and is still online, but some parts of it might function a bit strange (no maintenance done to it lately).

![A screenshot from a logged in user at teevy.co](/img/teevy_webapp.png)

The goal was to develop a Minimal Viable Product (MVP) first. We had no real business model for it, we are just passionate about what we do and this was just a hobby project for us to try new things.
...
The symbols `<$>` and `<*>` are just functions. I won't go much into detail, but these functions are related to Functor and Applicative. Aeson and Haskell can automatically derive the type class instance for the type `User`. Removing the need to write this boilerplate. Something like this:
...
The `FromRow` type class will extract columns from a result set and put them into a record. The order of the fields and selecting the right fields in the query is important!
